...
class Join:
    ...
    def build(self):
        ...
        while x <= self.numberOfSpillingPartitions:
            randWrite=math.floor(self.buildSize*real_expansion*1.0 / (self.numberOfPartitions)*1.0) - ((self.memoryInFrames - x + 1)*1.0 / (self.numberOfPartitions - x + 1)*1.0)+1
        ...
        if (self.level ==0):
            ...
            self.ophhj.inMemoryData = max(0,(self.buildSize - (self.numberOfSpillingPartitions*a)))
...
if __name__ == '__main__':
    ...
    buildSizes = [x*memory for x in [2,5,10,50,100,500,1000,15,20,15,20,30,40,50] ]
    #buildSizes=[20480]
